...
if [ -f "$INSTANCE_ENV" ]; then
  ...
  eval "$(
    ...
    for _ie_n in REPO_DIR KICKOFF_CORE_DIR MC_STATE_FILE MC_TRACKER_FILE \
                 ...
                 TELEGRAM_STATE_DIR REGROUND_PROMPT PERMISSION_MODE EFFORT \
...
announce_restart() {
  ...
  if printf 'url=%s\n' "$api_url" | curl -s -o /dev/null \
       ...
       --data-urlencode "text=🔄 Worker session restart #${count} — re-grounding…" \
       -K - 2>/dev/null; then
...
# "${PLUGIN_ARGS[@]}" is empty-array-safe under set -u on bash 4.4+ (this box: 5.2).
exec claude \
